...
{
    public partial class FrmRptListaCertificadosDefuncion : FrmActa
    {
        public FrmRptListaCertificadosDefuncion()
        {
            InitializeComponent();
        }

        private void FrmRptListaCertificadosDefuncion_Load(object sender, EventArgs e)
        {
            //-- Generamos la lista de Actas de Entrega
            CMetodos aMetodos = new CMetodos();
            DataTable Resultado = aMetodos.ListaCertificadosDefuncion();

            // -- Configurar el origen de datos del reporte
            ReportDataSource rds = new ReportDataSource
            {
                Name = "DsReportes_TListaCertificados",
                Value = Resultado
            };
            ...
            RutaRDLC = RutaRDLC.Substring(0, idx - 1) + @"\LibFormularios\ReportesRDLC\RptLista_CertificadosDefuncion.rdlc";
